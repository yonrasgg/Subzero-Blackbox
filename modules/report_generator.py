...
def _get_google_api_key() -> Optional[str]:
    """Get Google API key from config or environment variables."""
    import os
...
def generate_report(session: Session, job_type: str, job_id: int, run_stdout: str = "", run_stderr: str = "") -> str:
    ...
    if not api_key:
        return "Error: Google API key not configured. Please set 'google_api_key' in config/secrets.yaml."
...
"""
    ...
    try:
        ...
        try:
            ...
            report = f"""
**Executive Summary:** {report_json.get('executive_summary', 'N/A')}

**Detailed Findings:** {report_json.get('detailed_findings', 'N/A')}

**Recommendations:** {report_json.get('recommendations', 'N/A')}

**Conclusion:** {report_json.get('conclusion', 'N/A')}
"""
            return report
